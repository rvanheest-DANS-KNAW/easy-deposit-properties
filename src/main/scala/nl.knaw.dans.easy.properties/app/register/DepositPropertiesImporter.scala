...
object DepositPropertiesImporter {
  ...
  def importDepositProperties(props: DepositProperties, repo: Repository): MutationErrorOr[Unit] = {
    ...
    for {
      ...
      _ <- props.curator.traverse(repo.curator.store(depositId, _))
      _ <- props.isNewVersion.traverse(repo.isNewVersion.store(depositId, _))
      _ <- props.isCurationRequired.traverse(repo.isCurationRequired.store(depositId, _))
      _ <- props.isCurationPerformed.traverse(repo.isCurationPerformed.store(depositId, _))
